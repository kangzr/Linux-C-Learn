...
简单点来说就是单个进程/线程可同时监听和处理多个网络连接的socket fd，用户调用select/poll/epoll会阻塞用户进程，kernel会监听相关socket fd，当任意socket中数据准备好，select/poll/epoll则返回，用户进程再调用read操作，将数据从内核赋值到用户进程。因此会涉及两个系统调用select/poll/epoll和read。IO多路复用中需要将socket设置为non-blocking。即用户进程是被

上一篇文章我们介绍五种网络IO模型，如果是阻塞IO，则recvfrom接口会阻塞直到有数据copy完成，如果是单线程的话会导致主线程被阻塞，即整个程序永远锁死。当然可以通过多线程解决，即一个连接分配一个线程来处理，但是如果是百万连接情况，总不能创建百万个线程，毕竟操作系统的线程创建是有限的。因此就有了IO多路复用技术来解决单线程监听多个网络连接的情况。



#### 有哪些IO多路复用技术？

select, poll, epoll, kqueue等。接下来主要介绍select和epoll的实现原理。
