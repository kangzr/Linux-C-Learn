...
Python针对小对象（小于256字节）的内存分配采用内存池来进行管理，大对象直接使用标准C的内存分配器；
...
![python_block](..\pic\python_block.png)
...
![python_pool_4kb](..\pic\python_pool_4kb.png)
...
多个pool聚合的结果就是一个arena，每个arena大小为256KB，一个arena中容纳的pool个数就是64
...
![python_arena_state](..\pic\python_arena_state.png)
...
![python_arena_pool](..\pic\python_arena_pool.png)

Python小块内存全景图

![python_mem_full](..\pic\python_mem_full.png)
...
Python垃圾回收机制采用引用计数机制为主，标记-清除和分代回收机制为辅的策略，其中标记-清除机制用来解决计数引用带来的循环引用而无法释放内存的问题，分代回收机制是为了提升垃圾回收的效率。



##### 引用计数

优点：简单和实时；缺点：1. 执行回收效率低；2. 循环引用；



##### 标记-清除

用来解决引用技术机制产生的循环引用，进而导致内存泄漏的问题。循环引用只有在容器对象才回产生，比如字典，元组，列表等。

标记阶段：遍历所有的对象，如果是可达的(reachable)，也就是还有对象引用它，那么就标记该对象为可达

清除阶段：再次遍历对象，如果发现某个对象没有标记为可达(即unreachable)，则将其回收。

python基于三色模型来进行垃圾收集；（白色为未检测/不可达，灰色为可达，黑色为已检查）
