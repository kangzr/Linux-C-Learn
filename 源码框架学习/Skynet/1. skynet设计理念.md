...
- 栈上的都是lua类型的值
- **lua调用c**的函数得到一个新的栈，独立于之前的栈（不需要考虑栈的平衡）（每次生成不一样的栈）（**每次调用都有一个虚拟栈**）
- **c调用lua**，每一个协程都有一个虚拟栈，没有显示创建那么就是主协程上一个栈（需要维护栈的平衡）（栈跟协程绑定）（**c有多少个协程，每个协程一个虚拟栈**）
...
因为lua的协程同时只有一个在运行（一个协程调度另一个协程）

go csp协程 同时有多个协程在运行，因此可以做csp模型
...
local function accept(clientfd, addr)
	skynet.newservice("slave", clientfd, addr)
end

skynet.start(function ()
	local listenfd = socket.listen("0.0.0.0", 8001)
